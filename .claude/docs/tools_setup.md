...
**Installation:** Binary installed via mise (`aqua:dlvhdr/gh-dash` in `mise/tag-default/.config/mise/conf.d/cli.toml`), pulled in by `mise install` during `bootstrap`. Invoked as `gh-dash` (not `gh dash`).
...
- `mise run setup:zsh-config` — Configures `.zshrc`: injects mise activation block, completions (mise, gh), plugin list, and ZSH_THEME. Must run _after_ `setup:dotfiles` so `.zshrc` symlink is in place.
...
| ------------------------------------------------- | ------------------------------------------ | ----------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| File-based (`_<cmd>` in `~/.config/completions/`) | `mise`, `gh`                               | Generated by `setup:zsh-config`; refreshed in background each shell     | Tools whose completions evolve often _and_ whose output is `#compdef`-style                                                                                                                         |
...
- Per-machine tweaks (e.g. OS icon via `mise run setup:p10k-icon`) live in `~/.p10k.local.zsh`, sourced from `~/.zshrc` _after_ `~/.p10k.zsh` so they survive `p10k configure` regenerations and don't break the stow symlink to `~/.p10k.zsh`.
